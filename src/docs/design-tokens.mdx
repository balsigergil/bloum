...
| `--bl-clr-text`                | `gray-700`       | `gray-200`               | Primary text color                    |
...
Bloum ships its own copy of a Tailwind-compatible color palette — the values are
bundled in `bloum.css`, so **Tailwind is not required** to use them. It provides
the following color ranges:
...
Stacking order for layered components. Override these if Bloum's overlays need
to sit above (or below) something else in your app.
...
| Token Name               | Value | Description                       |
| :----------------------- | :---- | :-------------------------------- |
| `--bl-disabled-opacity`  | 0.7   | Opacity applied to disabled elements |
| `--bl-spinner-speed`     | 0.5s  | Spinner rotation duration         |
