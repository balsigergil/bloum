...
<div style={{display: "flex", gap: "1rem", flexWrap: "wrap"}}>
  ...
  <button className="button button--primary" onClick={() => {Toast.info('This toast won\'t disappear', { isClosable: true, duration: 0 })}}>Show toast that doesn't disappear</button>
</div>
...
<div>
  <button className="button button--primary" onClick={() => {Toast.success('Account created', { description: 'Your account has been successfully created.', isClosable: true })}}>Show info toast</button>
</div>
